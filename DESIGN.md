...
- **No priority tier on notifications.** The brief didn't ask for one and I resisted the urge to add it, because priority is not a single field — it is an architecture change. A `priority` string on the request without the underlying machinery would be cosmetic, and cosmetic priority is the exact failure mode where critical traffic still gets starved by bulk under load but everyone *thinks* it's protected. If I grew the service into needing priority, the shape would be: named Asynq queues (`critical`, `default`, `bulk`) with strict-priority or weighted drain (already a library-level knob); per-queue `WORKER_CONCURRENCY` budgets so a bulk blast can't consume every goroutine; rate-limit exemptions for the `critical` class (password resets must not 429 during an attack); the reconciler honoring the original priority on re-enqueue; and a `priority` label on `http_request_duration_seconds`, `channel_send_duration_seconds`, and `notifications_delivered_total` so each class has its own SLO. Cheap library-wise, load-bearing semantically — I'd only build it against a concrete SLO requirement, not speculatively.
